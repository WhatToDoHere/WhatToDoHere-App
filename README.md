...
<div align="center">
 <a href="#-links">WhatToDoHere iOS (App Store 배포 준비중)</a>
</div>
...
- [**개발 과정 톺아보기**](#개발-과정-톺아보기)
  ...
  - [**친구가 내 할 일 대신 등록하기**](#5-친구가-내-할-일-대신-등록하기)
  - [**App Store 배포 대응**](#6-app-store-배포-대응)
- [**회고**](#회고)
...
<div align="center">
<img width="1000" alt="Architecture" src="https://github.com/user-attachments/assets/eb35ee34-4bc7-49a2-b9c8-eac23308d866">
</div>
...
<br> -->
- 간편한 로그인을 위한 소셜(Google, Apple) 연동 로그인 및 로그아웃
  - 비회원 모드 제공, 위치 및 할 일 등록 기능 사용 가능
  - 로그인 시 비회원 모드의 로컬 데이터 자동 마이그레이션
  - 로그인 인증 상태 및 데이터 연속성 보장
...
**iOS 미리 알림**은 작업 알림에 대한 다양한 옵션을 제공하는 대표적인 Reminder 애플리케이션입니다. <br>날짜/시간/태그/위치 등을 알림에 설정할 수 있으나 개인적으로는 사용자가 단번에 이러한 기능을 모두 파악하기에는 어려움이 있다고 생각됐습니다. 그 이유가 무엇일까 생각했을 때, 미리 알림의 메인 화면이 폴더로 구성되어 있어 제공하는 정보가 극히 적고, 폴더에 태그나 위치정보를 등록할 수 있음에도 직접 설정해보지 않으면 알 수가 없었습니다.<br>반면, 인상깊었던 UI는 새로운 미리 알림을 등록하는 모달로 이동할 때 특유의 네이티브 인터랙션이 자연스러워 이런 점은 제 애플리케이션에도 적용하면 좋겠다고 생각했습니다.
<br><br>
**Todoist**는 메인 화면에서 타임라인 형식으로 작업 목록을 보여주고 있었습니다. 체크 리스트 형태여서 사용자가 즉시 완료 처리를 할 수 있다는 점이 간편해 보였고, 토글 기능을 이용해 목록을 접었다 펼칠 수 있는 기능이 사소하지만 편의를 제공한다고 느껴졌습니다.
...
2. **`NetInfo` 이벤트 리스너:**
   - 장점: WiFi 상태 변경을 실시간으로 감지 가능
   - 단점: 백그라운드에서 지원되지 않아 앱이 활성 상태일 때만 사용 가능
...
너무 가까운 위치를 별도로 등록하게 되면 사용자는 비슷한 위치에서 여러 알림을 받게 돼서 혼란스러울 수 있으며, 성능 측면에서도 더 많은 Geofence를 모니터링 해야하므로 배터리 소모도 증가할 수 있습니다. 따라서 근접한 위치의 경우 사용자에게 안내를 주고 최소 반경(300m)가 중복되지 않는 위치만 등록 가능하게 했습니다. 이를 위해 위도, 경도 데이터를 이용해 기존에 등록된 위치 중 근접한 위치가 있는지 비교하는 로직이 필요했습니다. 이때 고려한 알고리즘은 `Haversine formula` (하버사인 공식)과 `spherical law of cosines`(구면 코사인 법칙)입니다. 제가 필요한 것은 근접한 위치에 대한 거리였기 때문에 **하버사인 공식**을 이용하여 해결하였습니다. 그 이유는 다음과 같습니다.
...
| **계산 복잡성**         | 중간                                                                                                                                        | 낮음                                                                                                                                                 |
| **WhatToDoHere 적합성** | • 300m 이내의 근거리 계산에 매우 적합<br>• 간단한 구현으로 앱 성능에 부담 적음<br>• 위치 등록 및 WiFi 정보 관리에 이상적                    | • 앱의 주 용도인 근거리 계산에는 과도한 정밀도<br>• 각도 정보가 불필요한 현재 앱 요구사항에 비해 복잡                                                |
...
수많은 할 일 관리 앱을 보면 친구와 할 일을 단순 공유하는 기능은 있지만 친구가 대신 등록하는 기능은 없었습니다. <br> 예를 들어, <b>엄마가 "OO야, 집에 도착하면 전화해"</b>라는 메신저를 남겨놨다면, 이는 위치와 연관된 할 일이지만 사용자가 놓치기 쉬운 일입니다. 메신저를 확인 후 직접 할 일을 등록해야 하니까요. <br>이 때 엄마가 대신 사용자의 할 일을 WhatToDoHere 앱을 통해 등록해두면, 사용자가 집에 도착했을 때 엄마가 등록한 할 일에 대한 알림이 울릴테니 사용자가 할 일을 놓칠 가능성은 낮아지겠죠. 그래서 친구가 할 일을 대신 등록할 수 있는 기능을 구현하게 되었습니다.

### 5-1. 친구 검색용 유효성 검증 인풋 필드 만들기

## 6. App Store 배포 대응

### 6-1. 사용자가 꼭 로그인을 해야할까? 비회원 모드의 추가 구현
